...
st.caption("支持批量上传；每个文件可独立覆盖参数、独立分析范围与显示范围，并可查看20–32°双峰积分面积比例。")
...
with st.sidebar:
    ...
    with st.expander("📘 背景处理原理与教程", expanded=False):
        st.markdown("""
- **percentile（分位数背景）**：用低分位数近似常数背景，适合基线较平、背景起伏不大的谱线。  
- **rolling_min（滚动最小值）**：用滑动窗口最小值估计缓慢变化背景，适合有缓慢抬升底座的情况。  
- **poly（多项式背景）**：用低阶多项式拟合整体趋势，适合背景呈弯曲趋势的数据。  
- **asls（非对称最小二乘）**：对复杂宽背景更稳健，常用于拉曼/XRD基线扣除。  
- **none（不扣背景）**：直接使用原始强度，适合已经在外部完成背景处理的数据。  

**建议使用顺序**  
1. 先试 `percentile`，看峰位是否稳定；  
2. 若底座明显倾斜，再试 `rolling_min` 或 `poly`；  
3. 若背景复杂、峰底宽而不平，优先尝试 `asls`。  

**双峰分析建议**  
- 若 20–32° 区域视觉上确实有两个隆起，可开启双峰分解；  
- 若两个峰积分面积比例非常悬殊，或两个峰位特别接近，则要警惕“单宽峰被强行拆成双峰”。
""")
        ...
        with tab:
            ...
            if double_002 is not None:
                ...
                st.dataframe(pd.DataFrame({
                    ...
                    "积分面积": [double_002.peak_low.area, double_002.peak_high.area],
                    "积分面积比例": [double_002.area_ratio_low, double_002.area_ratio_high],
                }), use_container_width=True)
                st.caption("说明：低角峰通常对应更无序、层间距更大的组分；高角峰通常对应相对更有序的组分。")
            ...
            rows.append({
                ...
                "Area ratio low": (double_002.area_ratio_low if double_002 is not None else float("nan")),
                "Area ratio high": (double_002.area_ratio_high if double_002 is not None else float("nan")),
            })
